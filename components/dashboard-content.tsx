"use client"

import { useState } from "react"
import { Bell, ChevronDown, Menu, Search, MessageSquare, Pin, MoreHorizontal, X } from "lucide-react"
import { cn } from "@/lib/utils"
import Image from "next/image"
import Link from "next/link"

export default function DashboardContent() {
  const [sidebarOpen, setSidebarOpen] = useState(false)

  return (
    <div className="flex h-screen bg-gray-100">
      {/* Mobile sidebar overlay */}
      {sidebarOpen && (
        <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Sidebar */}
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 bg-white transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0",
          sidebarOpen ? "translate-x-0" : "-translate-x-full",
        )}
      >
        <div className="p-4 border-b">
          <div className="flex items-center">
            <h1 className="text-2xl font-bold">WeHR</h1>
            <button className="ml-auto md:hidden" onClick={() => setSidebarOpen(false)}>
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>

        <div className="p-4">
          <h3 className="text-xs uppercase text-gray-500 mb-4">Main Menu</h3>
          <nav className="space-y-2">
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-red-500 bg-red-50 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="3" y="3" width="7" height="7" rx="1" fill="currentColor" />
                  <rect x="3" y="14" width="7" height="7" rx="1" fill="currentColor" />
                  <rect x="14" y="3" width="7" height="7" rx="1" fill="currentColor" />
                  <rect x="14" y="14" width="7" height="7" rx="1" fill="currentColor" />
                </svg>
              </div>
              Dashboard
            </Link>
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M17 21V19C17 16.7909 15.2091 15 13 15H5C2.79086 15 1 16.7909 1 19V21"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                  <path
                    d="M9 11C11.2091 11 13 9.20914 13 7C13 4.79086 11.2091 3 9 3C6.79086 3 5 4.79086 5 7C5 9.20914 6.79086 11 9 11Z"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                  <path d="M23 21V19C22.9986 17.1771 21.765 15.5857 20 15.13" stroke="currentColor" strokeWidth="2" />
                  <path
                    d="M16 3.13C17.7699 3.58317 19.0078 5.17799 19.0078 7.005C19.0078 8.83201 17.7699 10.4268 16 10.88"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                </svg>
              </div>
              Recruitment
            </Link>
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="3" y="4" width="18" height="16" rx="2" stroke="currentColor" strokeWidth="2" />
                  <path d="M16 2V6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                  <path d="M8 2V6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                  <path d="M3 10H21" stroke="currentColor" strokeWidth="2" />
                </svg>
              </div>
              Schedule
            </Link>
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M20 21V19C20 16.7909 18.2091 15 16 15H8C5.79086 15 4 16.7909 4 19V21"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                  <path
                    d="M12 11C14.2091 11 16 9.20914 16 7C16 4.79086 14.2091 3 12 3C9.79086 3 8 4.79086 8 7C8 9.20914 9.79086 11 12 11Z"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                </svg>
              </div>
              Employee
            </Link>
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M10 3H3V10H10V3Z" stroke="currentColor" strokeWidth="2" />
                  <path d="M21 3H14V10H21V3Z" stroke="currentColor" strokeWidth="2" />
                  <path d="M21 14H14V21H21V14Z" stroke="currentColor" strokeWidth="2" />
                  <path d="M10 14H3V21H10V14Z" stroke="currentColor" strokeWidth="2" />
                </svg>
              </div>
              Department
            </Link>
          </nav>

          <h3 className="text-xs uppercase text-gray-500 mt-8 mb-4">Other</h3>
          <nav className="space-y-2">
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M21 11.5C21.0034 12.8199 20.6951 14.1219 20.1 15.3C19.3944 16.7118 18.3098 17.8992 16.9674 18.7293C15.6251 19.5594 14.0782 19.9994 12.5 20C11.1801 20.0035 9.87812 19.6951 8.7 19.1L3 21L4.9 15.3C4.30493 14.1219 3.99656 12.8199 4 11.5C4.00061 9.92179 4.44061 8.37488 5.27072 7.03258C6.10083 5.69028 7.28825 4.6056 8.7 3.90003C9.87812 3.30496 11.1801 2.99659 12.5 3.00003H13C15.0843 3.11502 17.053 3.99479 18.5291 5.47089C20.0052 6.94699 20.885 8.91568 21 11V11.5Z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </div>
              Support
            </Link>
            <Link href="#" className="flex items-center px-2 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded">
              <div className="w-6 h-6 mr-3 flex items-center justify-center">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M19.4 15C19.2669 15.3016 19.2272 15.6362 19.286 15.9606C19.3448 16.285 19.4995 16.5843 19.73 16.82L19.79 16.88C19.976 17.0657 20.1235 17.2863 20.2241 17.5291C20.3248 17.7719 20.3766 18.0322 20.3766 18.295C20.3766 18.5578 20.3248 18.8181 20.2241 19.0609C20.1235 19.3037 19.976 19.5243 19.79 19.71C19.6043 19.896 19.3837 20.0435 19.1409 20.1441C18.8981 20.2448 18.6378 20.2966 18.375 20.2966C18.1122 20.2966 17.8519 20.2448 17.6091 20.1441C17.3663 20.0435 17.1457 19.896 16.96 19.71L16.9 19.65C16.6643 19.4195 16.365 19.2648 16.0406 19.206C15.7162 19.1472 15.3816 19.1869 15.08 19.32C14.7842 19.4468 14.532 19.6572 14.3543 19.9255C14.1766 20.1938 14.0813 20.5082 14.08 20.83V21C14.08 21.5304 13.8693 22.0391 13.4942 22.4142C13.1191 22.7893 12.6104 23 12.08 23C11.5496 23 11.0409 22.7893 10.6658 22.4142C10.2907 22.0391 10.08 21.5304 10.08 21V20.91C10.0723 20.579 9.96512 20.258 9.77251 19.9887C9.5799 19.7194 9.31074 19.5143 9 19.4C8.69838 19.2669 8.36381 19.2272 8.03941 19.286C7.71502 19.3448 7.41568 19.4995 7.18 19.73L7.12 19.79C6.93425 19.976 6.71368 20.1235 6.47088 20.2241C6.22808 20.3248 5.96783 20.3766 5.705 20.3766C5.44217 20.3766 5.18192 20.3248 4.93912 20.2241C4.69632 20.1235 4.47575 19.976 4.29 19.79C4.10405 19.6043 3.95653 19.3837 3.85588 19.1409C3.75523 18.8981 3.70343 18.6378 3.70343 18.375C3.70343 18.1122 3.75523 17.8519 3.85588 17.6091C3.95653 17.3663 4.10405 17.1457 4.29 16.96L4.35 16.9C4.58054 16.6643 4.73519 16.365 4.794 16.0406C4.85282 15.7162 4.81312 15.3816 4.68 15.08C4.55324 14.7842 4.34276 14.532 4.07447 14.3543C3.80618 14.1766 3.49179 14.0813 3.17 14.08H3C2.46957 14.08 1.96086 13.8693 1.58579 13.4942C1.21071 13.1191 1 12.6104 1 12.08C1 11.5496 1.21071 11.0409 1.58579 10.6658C1.96086 10.2907 2.46957 10.08 3 10.08H3.09C3.42099 10.0723 3.742 9.96512 4.0113 9.77251C4.28059 9.5799 4.48572 9.31074 4.6 9C4.73312 8.69838 4.77282 8.36381 4.714 8.03941C4.65519 7.71502 4.50054 7.41568 4.27 7.18L4.21 7.12C4.02405 6.93425 3.87653 6.71368 3.77588 6.47088C3.67523 6.22808 3.62343 5.96783 3.62343 5.705C3.62343 5.44217 3.67523 5.18192 3.77588 4.93912C3.87653 4.69632 4.02405 4.47575 4.21 4.29C4.39575 4.10405 4.61632 3.95653 4.85912 3.85588C5.10192 3.75523 5.36217 3.70343 5.625 3.70343C5.88783 3.70343 6.14808 3.75523 6.39088 3.85588C6.63368 3.95653 6.85425 4.10405 7.04 4.29L7.1 4.35C7.33568 4.58054 7.63502 4.73519 7.95941 4.794C8.28381 4.85282 8.61838 4.81312 8.92 4.68H9C9.29577 4.55324 9.54802 4.34276 9.72569 4.07447C9.90337 3.80618 9.99872 3.49179 10 3.17V3C10 2.46957 10.2107 1.96086 10.5858 1.58579C10.9609 1.21071 11.4696 1 12 1C12.5304 1 13.0391 1.21071 13.4142 1.58579C13.7893 1.96086 14 2.46957 14 3V3.09C14.0013 3.41179 14.0966 3.72618 14.2743 3.99447C14.452 4.26276 14.7042 4.47324 15 4.6C15.3016 4.73312 15.6362 4.77282 15.9606 4.714C16.285 4.65519 16.5843 4.50054 16.82 4.27L16.88 4.21C17.0657 4.02405 17.2863 3.87653 17.5291 3.77588C17.7719 3.67523 18.0322 3.62343 18.295 3.62343C18.5578 3.62343 18.8181 3.67523 19.0609 3.77588C19.3037 3.87653 19.5243 4.02405 19.71 4.21C19.896 4.39575 20.0435 4.61632 20.1441 4.85912C20.2448 5.10192 20.2966 5.36217 20.2966 5.625C20.2966 5.88783 20.2448 6.14808 20.1441 6.39088C20.0435 6.63368 19.896 6.85425 19.71 7.04L19.65 7.1C19.4195 7.33568 19.2648 7.63502 19.206 7.95941C19.1472 8.28381 19.1869 8.61838 19.32 8.92V9C19.4468 9.29577 19.6572 9.54802 19.9255 9.72569C20.1938 9.90337 20.5082 9.99872 20.83 10H21C21.5304 10 22.0391 10.2107 22.4142 10.5858C22.7893 10.9609 23 11.4696 23 12C23 12.5304 22.7893 13.0391 22.4142 13.4142C22.0391 13.7893 21.5304 14 21 14H20.91C20.5882 14.0013 20.2738 14.0966 20.0055 14.2743C19.7372 14.452 19.5268 14.7042 19.4 15Z"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </div>
              Settings
            </Link>
          </nav>
        </div>
      </aside>

      {/* Main content */}
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top navigation */}
        <header className="bg-white border-b p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <button className="md:hidden mr-4" onClick={() => setSidebarOpen(true)}>
                <Menu className="h-5 w-5" />
              </button>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Search className="h-5 w-5 text-gray-400" />
                </div>
                <input
                  type="text"
                  placeholder="Search"
                  className="pl-10 pr-4 py-2 border rounded-lg w-full md:w-auto focus:outline-none focus:ring-2 focus:ring-gray-200"
                />
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <button className="relative text-gray-500 hover:text-gray-700">
                <Bell className="h-5 w-5" />
                <span className="absolute top-0 right-0 h-2 w-2 bg-red-500 rounded-full"></span>
              </button>
              <button className="relative text-gray-500 hover:text-gray-700">
                <MessageSquare className="h-5 w-5" />
              </button>
              <div className="flex items-center">
                <div className="relative">
                  <Image
                    src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-FqWSuJyiRJTNFAEAUUAMNrroqUTQ6D.png"
                    alt="Profile"
                    width={36}
                    height={36}
                    className="rounded-full"
                  />
                  <div className="absolute bottom-0 right-0 bg-green-500 rounded-full h-2.5 w-2.5 border-2 border-white"></div>
                </div>
                <div className="ml-2 hidden md:block">
                  <p className="text-sm font-medium">Admirra John</p>
                </div>
                <ChevronDown className="h-4 w-4 ml-1 text-gray-500" />
              </div>
            </div>
          </div>
        </header>

        {/* Main dashboard content */}
        <main className="flex-1 overflow-y-auto p-4 bg-gray-100">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-2xl font-bold mb-6">Dashboard</h1>

            {/* Dashboard cards - first row */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {/* Available Position */}
              <div className="bg-white rounded-lg shadow p-6 border-t-4 border-red-200 bg-gradient-to-br from-red-50 to-white">
                <h3 className="font-medium mb-2">Available Position</h3>
                <p className="text-3xl font-bold mb-2">24</p>
                <p className="text-sm text-red-500">4 Urgently needed</p>
              </div>

              {/* Job Open */}
              <div className="bg-white rounded-lg shadow p-6 border-t-4 border-blue-200 bg-gradient-to-br from-blue-50 to-white">
                <h3 className="font-medium mb-2">Job Open</h3>
                <p className="text-3xl font-bold mb-2">10</p>
                <p className="text-sm text-blue-500">4 Active hiring</p>
              </div>

              {/* New Employees */}
              <div className="bg-white rounded-lg shadow p-6 border-t-4 border-purple-200 bg-gradient-to-br from-purple-50 to-white">
                <h3 className="font-medium mb-2">New Employees</h3>
                <p className="text-3xl font-bold mb-2">24</p>
                <p className="text-sm text-purple-500">4 Department</p>
              </div>
            </div>

            {/* Dashboard cards - second row */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {/* Total Employees */}
              <div className="bg-white rounded-lg shadow p-6">
                <h3 className="font-medium mb-2">Total Employees</h3>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-3xl font-bold">216</p>
                  <div className="flex items-center">
                    <svg className="h-5 w-5 text-green-500 mr-1" viewBox="0 0 24 24" fill="none">
                      <path
                        d="M18 15L12 9L6 15"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                    </svg>
                    <span className="text-sm text-green-500">+2% Past month</span>
                  </div>
                </div>
                <div className="h-10 relative">
                  <svg className="w-full h-full" viewBox="0 0 200 40" fill="none" preserveAspectRatio="none">
                    <path d="M0,20 Q50,5 100,20 T200,20" stroke="#ef4444" strokeWidth="2" fill="none" />
                  </svg>
                </div>
                <div className="flex justify-between mt-2 text-xs text-gray-500">
                  <div>120 Men</div>
                  <div>96 Women</div>
                </div>
              </div>

              {/* Talent Request */}
              <div className="bg-white rounded-lg shadow p-6">
                <h3 className="font-medium mb-2">Talent Request</h3>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-3xl font-bold">16</p>
                  <div className="flex items-center">
                    <svg className="h-5 w-5 text-green-500 mr-1" viewBox="0 0 24 24" fill="none">
                      <path
                        d="M18 15L12 9L6 15"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                    </svg>
                    <span className="text-sm text-green-500">+5% Past month</span>
                  </div>
                </div>
                <div className="h-10 relative">
                  <svg className="w-full h-full" viewBox="0 0 200 40" fill="none" preserveAspectRatio="none">
                    <path d="M0,20 Q50,5 100,20 T200,20" stroke="#ef4444" strokeWidth="2" fill="none" />
                  </svg>
                </div>
                <div className="flex justify-between mt-2 text-xs text-gray-500">
                  <div>6 Men</div>
                  <div>10 Women</div>
                </div>
              </div>
            </div>

            {/* Last row - Announcements, Activity, Schedule */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Announcements */}
              <div className="bg-white rounded-lg shadow">
                <div className="flex items-center justify-between p-6 border-b">
                  <h3 className="font-medium">Announcement</h3>
                  <div className="text-sm text-gray-500">Today, 13 Sep 2021</div>
                </div>
                <div className="p-6">
                  <div className="space-y-6">
                    <div className="flex">
                      <div className="flex-1">
                        <p className="font-medium">Outing schedule for every department</p>
                        <p className="text-sm text-gray-500 mt-1">5 Minutes ago</p>
                      </div>
                      <div className="flex space-x-2">
                        <button className="text-gray-400 hover:text-gray-600">
                          <Pin className="h-4 w-4" />
                        </button>
                        <button className="text-gray-400 hover:text-gray-600">
                          <MoreHorizontal className="h-4 w-4" />
                        </button>
                      </div>
                    </div>

                    <div className="flex">
                      <div className="flex-1">
                        <p className="font-medium">Meeting HR Department</p>
                        <p className="text-sm text-gray-500 mt-1">Yesterday, 10:30 PM</p>
                      </div>
                      <div className="flex space-x-2">
                        <button className="text-gray-400 hover:text-gray-600">
                          <Pin className="h-4 w-4" />
                        </button>
                        <button className="text-gray-400 hover:text-gray-600">
                          <MoreHorizontal className="h-4 w-4" />
                        </button>
                      </div>
                    </div>

                    <div className="flex">
                      <div className="flex-1">
                        <p className="font-medium">IT Department need two more talents for UX/UI Designer position</p>
                        <p className="text-sm text-gray-500 mt-1">Yesterday, 09:15 AM</p>
                      </div>
                      <div className="flex space-x-2">
                        <button className="text-gray-400 hover:text-gray-600">
                          <Pin className="h-4 w-4" />
                        </button>
                        <button className="text-gray-400 hover:text-gray-600">
                          <MoreHorizontal className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
                  </div>

                  <div className="mt-4 text-center">
                    <button className="text-red-500 text-sm font-medium">See All Announcement</button>
                  </div>
                </div>
              </div>

              <div className="grid grid-cols-1 gap-4">
                {/* Recently Activity */}
                <div className="bg-navy-900 text-white rounded-lg shadow p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="font-medium">Recently Activity</h3>
                  </div>
                  <div className="text-xs text-gray-400 mb-2">10:40 AM, 10 SEP 2021</div>
                  <h4 className="text-lg font-medium mb-2">You Posted a New Job</h4>
                  <p className="text-sm mb-4">
                    Kindly check the requirements and terms of work and make sure everything is right.
                  </p>

                  <div className="flex justify-between items-center">
                    <div className="text-sm">Today you makes 12 Activity</div>
                    <button className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md text-sm">
                      See All Activity
                    </button>
                  </div>
                </div>

                {/* Upcoming Schedule */}
                <div className="bg-white rounded-lg shadow">
                  <div className="flex items-center justify-between p-6 border-b">
                    <h3 className="font-medium">Upcoming Schedule</h3>
                    <div className="text-sm text-gray-500">Today, 13 Sep 2021</div>
                  </div>
                  <div className="p-6">
                    <div className="space-y-6">
                      <div>
                        <div className="text-xs font-medium text-gray-500 mb-2">Priority</div>
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-medium">Review candidate applications</h4>
                            <p className="text-sm text-gray-500 mt-1">Today, 11:30 AM</p>
                          </div>
                          <button className="text-gray-400 hover:text-gray-600">
                            <MoreHorizontal className="h-4 w-4" />
                          </button>
                        </div>
                      </div>

                      <div>
                        <div className="text-xs font-medium text-gray-500 mb-2">Other</div>
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-medium">Interview with candidates</h4>
                            <p className="text-sm text-gray-500 mt-1">Today, 10:30 AM</p>
                          </div>
                          <button className="text-gray-400 hover:text-gray-600">
                            <MoreHorizontal className="h-4 w-4" />
                          </button>
                        </div>
                      </div>

                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium">Short meeting with product designer from IT Department</h4>
                          <p className="text-sm text-gray-500 mt-1">Today, 09:15 AM</p>
                        </div>
                        <button className="text-gray-400 hover:text-gray-600">
                          <MoreHorizontal className="h-4 w-4" />
                        </button>
                      </div>
                    </div>

                    <div className="mt-4 text-center">
                      <button className="text-red-500 text-sm font-medium">Create a New Schedule</button>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </main>
      </div>
    </div>
  )
}

